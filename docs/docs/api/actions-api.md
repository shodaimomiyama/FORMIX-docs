...
- `FormixClient` - High-level client with builder-pattern API
...
Re-encrypted capsule fragment generated by Holder-Process. Contains sensitive cryptographic data.
...
pub struct CFrag {
    ...
    cfrag_data: Vec<u8>,               // Re-encrypted fragment (SENSITIVE)
    created_at: u64,
}
```

**Security**: Implements `Zeroize` and `ZeroizeOnDrop` to clear sensitive `cfrag_data` from memory.
